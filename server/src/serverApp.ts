...
export function createCollabServer(options: CollabServerOptions): CollabServer {
  ...
  const store = new PersistentRoomStore(options.dbPath);
  ...
  io.on("connection", (socket: Socket) => {
    socket.on(
      "create-room",
      (
        rawPayload: CreateRoomPayload,
        acknowledge?: (response: CreateRoomResponse) => void,
      ) => {
        const payload = sanitizeCreateRoomPayload(rawPayload);
        if (!payload) {
          acknowledgeCreateRoom(
            acknowledge,
            createErrorResponse("invalid-payload", "Invalid room creation payload."),
          );
          return;
        }
        ...
        if (versionError) {
          acknowledgeCreateRoom(
            acknowledge,
            createErrorResponse(versionError.code, versionError.message),
          );
          return;
        }
        ...
        if (!rateLimit.allowed) {
          acknowledgeCreateRoom(
            ...
            createErrorResponse(
              "rate-limited",
              "Too many room creation requests. Please wait a little and try again.",
        ...
        if (roomManager.getUser(socket.id)) {
          acknowledgeCreateRoom(
            ...
            createErrorResponse(
              "invalid-payload",
              "This client is already joined to a room.",
            ),
          );
          return;
        }
        ...
        if (!room) {
          acknowledgeCreateRoom(
            ...
            createErrorResponse(
              "room-unavailable",
              "That room ID is already in use.",
            ),
          );
          return;
        }

        const joinResult = roomManager.addUser(
          socket.id,
          room.id,
          payload.userName,
          "teacher",
        ...
        if (!joinResult.ok) {
          ...
          acknowledgeCreateRoom(
            ...
            createErrorResponse(
              "room-unavailable",
              "Unable to add the teacher to the room.",
            ),
          );
          return;
        }
        ...
        if (!roomState) {
          ...
          acknowledgeCreateRoom(
            ...
            createErrorResponse(
              "room-unavailable",
              "Unable to initialize the room state.",
            ),
          );
          return;
        }

        const teacherInviteToken = createInviteToken(
          room.id,
          "teacher",
          options.inviteSecret,
          options.inviteTokenTtlHours,
        );
        const studentInviteToken = createInviteToken(
          room.id,
          "student",
          options.inviteSecret,
          options.inviteTokenTtlHours,
        ...
        console.log(
          `[Room] ${joinResult.user.name} joined ${room.id} as ${joinResult.user.role} (${joinResult.room.users.size} active user(s))`,
        );
      },
    );
    ...
    socket.on("join-room", (rawPayload: JoinRoomPayload) => {
      ...
      if (!rateLimit.allowed) {
        emitProtocolError(
          socket,
          "rate-limited",
          "Too many join requests. Please wait a little and try again.",
          rateLimit.retryAfterMs,
        );
        return;
      }
      ...
      const inviteClaims = verifyInviteToken(payload.inviteToken, options.inviteSecret);
      if (!inviteClaims) {
        emitProtocolError(
          socket,
          "invite-invalid-or-expired",
          "That invite token is invalid or expired.",
        );
        return;
      }
      ...
      if (!joinResult.ok) {
        ...
        if (joinResult.reason === "teacher-already-present") {
          emitProtocolError(
            ...
            "A teacher is already connected to this room.",
          );
          return;
        }
        ...
        emitProtocolError(
          socket,
          "room-full",
          `That room is full. A maximum of ${options.maxUsersPerRoom} users can join at once.`,
        );
        return;
      }
      ...
      console.log(
        `[Room] ${user.name} joined ${room.id} as ${user.role} (${room.users.size} active user(s))`,
      );
    });
    ...
    socket.on("yjs-update", (rawPayload: YjsUpdatePayload) => {
      ...
      if (!applied) {
        emitProtocolError(
          socket,
          "room-unavailable",
          "Unable to apply the document update.",
        );
        return;
      }
    ...
    socket.on("cursor-update", (rawPayload: CursorUpdatePayload) => {
      ...
      if (!rateLimit.allowed) {
        emitProtocolError(
          socket,
          "rate-limited",
          "Cursor updates are arriving too quickly. Please slow down for a moment.",
          rateLimit.retryAfterMs,
        );
        return;
      }
...
function acknowledgeCreateRoom(
  acknowledge: ((response: CreateRoomResponse) => void) | undefined,
...
function enrichRoomState(
  payload: Omit<RoomStatePayload, "protocolVersion" | "serverVersion">,
): RoomStatePayload {
  return {
    ...payload,
    protocolVersion: PROTOCOL_VERSION,
...
function sanitizeCreateRoomPayload(
  payload: CreateRoomPayload | null | undefined,
...
function sanitizeJoinPayload(
  payload: JoinRoomPayload | null | undefined,
...
function validateClientVersion(clientVersion: string): ProtocolErrorPayload | null {
  const clientMajor = Number.parseInt(clientVersion.split(".")[0] ?? "", 10);
  const serverMajor = Number.parseInt(SERVER_VERSION.split(".")[0] ?? "", 10);

  if (!Number.isInteger(clientMajor) || clientMajor !== serverMajor) {
    return {
      code: "protocol-mismatch",
      message: `This CollabCode build is not compatible with the server. Install CollabCode ${SERVER_VERSION} (protocol ${PROTOCOL_VERSION}).`,
    };
  }
